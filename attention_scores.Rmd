...
# For partial matching, `DISEASE/TRAIT is added (when the GDP terms' identifiers are not matched with GWAS's identifiers, we will use (`DISEASE/TRAIT`) column for partial matching )
...
# removing duplicates in the gbd_long by "GDB term" not MAPPED_TRAIT_URI
unmatched_gbd <- anti_join(gbd_long, attention, by = "MAPPED_TRAIT_URI") %>%
...
Ensure consistent formatting and standardize of DISEASE/TRAIT in unmatched GBD terms and unmatched attention ie., both lower case for comparison 
...
# Perform partial matching between MAPPED_TRAIT_NAME in unmatched_attention and GBD_TERM_NAME in gbd_long without identifiers 

#partial_matches <- unmatched_gbd %>%
  cross_join(unmatched_attention) %>%
  filter(str_detect(MAPPED_TRAIT_NAME, GBD_TERM_NAME)) %>%
  select(GBD_TERM_NAME, MAPPED_TRAIT_NAME) %>%
  distinct()


# with identifiers in both GWAS and GDB 
  
partial_matches <- unmatched_gbd %>%
  cross_join(unmatched_attention) %>%
  filter(str_detect(MAPPED_TRAIT_NAME, GBD_TERM_NAME)) %>%
  select(GBD_TERM_NAME, MAPPED_TRAIT_NAME, GBD_TRAIT_URI = MAPPED_TRAIT_URI.x, ATTENTION_TRAIT_URI = MAPPED_TRAIT_URI.y) %>%
  distinct()


partial_matches_unique <- partial_matches %>%
  distinct(GBD_TERM_NAME, .keep_all = TRUE)
print(head(partial_matches, 20))
...
Finally identify the GDB terms that are not mapped via identifiers or trait with GWAS
