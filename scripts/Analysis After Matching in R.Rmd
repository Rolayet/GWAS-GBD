...
top_causes <- dataset_Ncase %>%
  group_by(`Cause Name`) %>%
  summarise(total_attention_score = sum(total_attention_score)) %>%
...
dataset_Ncase %>%
  ...
  summarise(
    count = n(),
    sum_score = sum(total_attention_score),
    has_na = any(is.na(total_attention_score)),
    has_zero = any(total_attention_score == 0)
  )
...
# View the top causes sorted by DALY with rank
print(top_causes)

#write.csv(top_causes, "top_causes_with_daly_rank.csv", row.names = FALSE)
```
...
#write.csv(top_causes, "top_GBD_causes_with_daly_rank.csv", row.names = FALSE)
