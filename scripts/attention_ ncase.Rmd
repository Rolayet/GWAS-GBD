...
#  Group by MAPPED_TRAIT_URI and PUBMEDID, then summarize
intermediate_summary <- a %>%
...
# Save or print the intermediate result if needed
print(intermediate_summary)

# Group by MAPPED_TRAIT_URI and finalize the summary
attention <- intermediate_summary %>%
...
# remove the columns
combined_dataset <- combined_dataset %>% select(-MAPPED_TRAIT_NAME, -GBD_TERM_NAME, -EFO_number, -MAPPED_TRAIT_URI)
...
#write.csv(combined_dataset, "combined_dataset_ncaseM.csv", row.names = FALSE)
...
write.csv(filtered_dataset, "filtered_dataset_10.csv", row.names = FALSE)
...
```{r}
merged_dataset_exclude_Injuries <- filtered_dataset

#write.csv(merged_dataset_exclude_Injuries, "merged_dataset_exclude_Injuries.csv", row.names = FALSE)
```
